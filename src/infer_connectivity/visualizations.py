...
def visualize_connectivity_matrix(G, N_E):
    ...
    ax1.text((N_E + len(G)) / 2, -0.1 * len(G), 'Inhibitory', ha='center', transform=ax1.transData)
    ...
    ax2.bar(connection_types, connection_counts,
        color=['red', 'purple', 'orange', 'blue'], alpha=0.7)

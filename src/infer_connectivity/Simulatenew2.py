...
def Simulate(
    ...
    UpdatedGraph, spikes, spikestime = step(
        G,
        0,
        spikes,
        spikestime,
    ...
    while count < sec * 10000:
        ...
        if count % 500 == 0:
            if count != 20000:
                ...
                print(
                    str((spikesExc1) / (((count / 10000) - 2) * NE))
                    + ","
                    + str((spikesInh) / (((count / 10000) - 2) * NI))
                )
        count += 1.0
    return spikes
...
def run_simulation_with_params(
    ...
    for i in range(NE + NI):
        if i < NE:
            spikestime[i] = [
                random.randint(EEdela, EEdel) if x < NE else 10 for x in range(NE + NI)
            ]

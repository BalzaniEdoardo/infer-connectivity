...
#Simulates neuronal firing similar to "Emergent spike patterns in neuronal populations" by L. Chariker and L. Young, and "Malleability of gamma rhythms enhances population-level correlation" by S. Saraf and L. Young
...
def Simulate(G, spikes, spikestime, spikescounter, refrac, EEdel, IEdel, EIdel, IIdel, Edrive, Idrive, SEE, SIE, SEI, SII, TauE, TauI, NE, NI, sec):
  ...
  UpdatedGraph, spikes, spikestime = step(G, 0, spikes, spikestime, spikescounter, refrac, EEdel, IEdel, EIdel, IIdel, Edrive, Idrive, SEE, SIE, SEI, SII, TauE, TauI, NE, NI)
  count = 1.0
  voltagesE = 0.0
  voltagesI = 0.0
  while count<sec*10000:
    UpdatedGraph, spikes, spikestime = step(UpdatedGraph, count, spikes, spikestime, spikescounter, refrac, EEdel, IEdel, EIdel, IIdel, Edrive, Idrive, SEE, SIE, SEI, SII, TauE, TauI, NE, NI)
    if count % 500==0:
      if count != 20000:
        spikesExc1 = len(sum(spikes[:NE], []))
        spikesInh = len(sum(spikes[NE:NE+NI], []))
        print(str((spikesExc1)/(((count/10000)-2)*NE))+ "," + str((spikesInh)/(((count/10000)-2)*NI)))
    count+=1.0
  return spikes

def step(G, count, spikes, spikestime, spikescounter, refrac, EEdel, IEdel, EIdel, IIdel, Edrive, Idrive, SEE, SIE, SEI, SII, TauE, TauI, NE, NI):
...
if __name__ == "__main__":
 # inputs = raw_input("Enter refractory, EE delay, IE delay, EI delay, II delay, Edrive, I drive, SEE, SIE, SEI, SII, Tau_E, Tau_I, drive constant E, drive constant I, NE, NI, sec: ")
   # inputs = sys.argv[1:]
  ...
  #  print(str(CE) + " " + str(CI))
    ...
    spikes = Simulate(G, spikes, spikestime, spikescounter, refrac, EEdel, IEdel, EIdel, IIdel, Edrive, Idrive, SEE, SIE, SEI, SII, TauE, TauI, NE, NI, sec)

...
spikes_tsgroup = nap.TsGroup({n: nap.Ts(np.array(spikes[n])/1000) for n in range(len(spikes))})
...
param_grid = {
    "regularizer_strength":
        np.geomspace(10**-8, 10**-3, 8) if regularizer != "UnRegularized"
        else None
}
...
if 'param_regularizer_strength' in cls.cv_results_:
    cv_data['param_regularizer_strength'] = np.array(cls.cv_results_['param_regularizer_strength'])
...
cv_path = output_dir / f"cv_results_dataset_{dataset_path.stem}_neuron_{neuron_fit}_config_{conf_path.stem}.npz"
np.savez(cv_path, **cv_data)

metadata_path = output_dir / f"metadata_{dataset_path.stem}_neuron_{neuron_fit}_config_{conf_path.stem}.json"

with open(metadata_path, 'w') as f:
    json.dump(metadata, f, indent=2)

model_path = output_dir / f"best_model_{dataset_path.stem}_neuron_{neuron_fit}_config_{conf_path.stem}.npz"
